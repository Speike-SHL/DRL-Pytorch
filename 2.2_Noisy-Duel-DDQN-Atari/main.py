"""
本节噪声网络最主要部分看 utils 中的 NoisyLinear 类,
次要部分看 Agent 中的 Q_Net 和 Duel_Q_Net 类
"""
...
parser.add_argument(
    "--random_steps", type=int, default=int(1e4), help="random steps before training, 5E4 in DQN Nature"
)
...
parser.add_argument(
    "--anneal_frac", type=int, default=3e5, help="anneal_frac steps, make epsilon from init_e to final_e"
)
...
parser.add_argument("--Duel", type=str2bool, default=True, help="whether to use Duel. Q-learning")
parser.add_argument("--Noisy", type=str2bool, default=True, help="whether to use NoisyNet")
opt = parser.parse_args()
opt.dvc = torch.device(opt.device if torch.cuda.is_available() else "cpu")
opt.algo_name = (
    ("Double-" if opt.Double else "") + ("Duel-" if opt.Duel else "") + ("Noisy-" if opt.Noisy else "") + "DQN"
)
...
def main():
    ...
    # 使用包装器创建环境, 包装器见 tianshou_wrappers.py.
    # QUERY 这里创建的评估环境, 这个环境不需要包装吗？为什么都是False
    eval_env = make_env_tianshou(
        opt.EnvName, noop_reset=opt.noop_reset, episode_life=False, clip_rewards=False, render_mode=render_mode
    )
    opt.action_dim = eval_env.action_space.n
    print(
        "Algorithm:", opt.algo_name, "  Env:", opt.EnvName, "  Action_dim:", opt.action_dim, "  Seed:", opt.seed, "\n"
    )

    # Build Agent
    if not os.path.exists("./2.2_Noisy-Duel-DDQN-Atari/model"):
        os.mkdir("./2.2_Noisy-Duel-DDQN-Atari/model")
    ...
    if opt.render:
        ...
        if opt.write:
            ...
            writepath = f"./2.2_Noisy-Duel-DDQN-Atari/runs/{opt.ExperimentName}_S{opt.seed}" + timenow
        ...
        # 探索噪声在 anneal_frac 步内从 1.0 线性退火到 0.02
