...
class DQN_Agent(object):
    ...
    def train(self, replay_buffer):
        ...
        # BP
        # tr 的话, 这个样本是无用的。 loss 手动定义为平方根损失 * 权重
        ...
        # 重新更新优先级
        with torch.no_grad():
            batch_priorities = ((torch.abs(Q_target - current_Q) + 0.01) ** replay_buffer.alpha).squeeze(
                -1
            )  # (batchsize,) on devive
    ...
    def load(self, algo, EnvName, steps):
        ...
        self.q_target.load_state_dict(
            torch.load("./model/{}_{}_{}.pth".format(algo, EnvName, steps), map_location=device)
        )
