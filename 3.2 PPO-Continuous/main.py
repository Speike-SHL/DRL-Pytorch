...
parser.add_argument("--EnvIdex", type=int, default=0, help="Pend-v1, LLd-cv2, Human-v4, HC-v4, BW-v3, BWH-v3")
...
def main():
    ...
    BrifEnvName = ["Pend-v1", "LLd-cv2", "Human-v4", "HC-v4", "BW-v3", "BWH-v3"]
    ...
    opt.max_action = float(env.action_space.high[0])  # 动作取值范围的上下界
    ...
    if opt.write:
        ...
        writepath = "3.2 PPO-Continuous/runs/{}".format(BrifEnvName[opt.EnvIdex]) + timenow
    ...
    if not os.path.exists("3.2 PPO-Continuous/model"):
        os.mkdir("3.2 PPO-Continuous/model")
